if status is-interactive
    ...
    echo  >> ~/vttest
    if [ $XDG_VTNR -eq 1 ] && not set -q TMUX && [ "$TERM_PROGRAM" != "vscode" ] && [ -z "$VSCODE_RESOLVING_ENVIRONMENT" ]
        ...
        if [ -n "$UNATTACHED_SESSION_ID" ] && [ (pwd) = "$HOME"  ]
...
fish_add_path -m ~/.local/share/vcpkg/
