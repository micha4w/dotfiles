...
import { bulkConnect, loadInterfaceXML } from 'resource:///com/github/Aylur/ags/utils.js';
...
const DbusmenuGtk3Menu = Widget(DbusmenuGtk3.Menu);
...
export class TrayItem extends Service {
    ...
    /** @type {import('dbus/types').StatusNotifierItemProxy} */
    _proxy;
    _busName = undefined;

    _iconTheme = undefined;
    /** @type {Gtk.Menu | undefined} */
    menu = undefined;
    ...
    constructor(busName, objectPath) {
        ...
        this._proxy = new StatusNotifierItemProxy(Gio.DBus.session, busName, objectPath, this._itemProxyAcquired.bind(this), null, Gio.DBusProxyFlags.NONE);
    }

    /**
     * @default (event: Gdk.Event) => {
     *         this._proxy.ActivateAsync(event.get_root_coords()[1], event.get_root_coords()[2]);
     *     }
     */
    activate = (event) => {
        this._proxy.ActivateAsync(event.get_root_coords()[1], event.get_root_coords()[2]).catch((e) => {
            if (e instanceof GLib.Error && e.matches(Gio.dbus_error_quark(), Gio.DBusError.UNKNOWN_METHOD)) return;
    ...
    secondaryActivate = (event) => {
        this._proxy.SecondaryActivateAsync(event.get_root_coords()[1], event.get_root_coords()[2]).catch(console.error).catch((e) => {
            if (e instanceof GLib.Error && e.matches(Gio.dbus_error_quark(), Gio.DBusError.UNKNOWN_METHOD)) return;
...
;
    ...
    scroll = (event) => {
        ...
        this._proxy.ScrollAsync(delta, direction).catch((e) => {
            if (e instanceof GLib.Error && e.matches(Gio.dbus_error_quark(), Gio.DBusError.UNKNOWN_METHOD)) return;
...
;
    ...
    get icon() {
        ...
        if (this._iconTheme && iconName) {
            ...
            const iconInfo = this._iconTheme.lookup_icon(iconName, size, Gtk.IconLookupFlags.FORCE_SIZE);
    ...
    /**
     * @param {StatusNotifierItemProxy} proxy
     * @returns {void}
     */
    ...
    _refreshAllProperties() {
        this._proxy.g_connection.call(this._proxy.g_name, this._proxy.g_object_path, 'org.freedesktop.DBus.Properties', 'GetAll', new GLib.Variant('(s)', [this._proxy.g_interface_name]), new GLib.VariantType('(a{sv})'), Gio.DBusCallFlags.NONE, -1, null, (proxy, result) => {
    ...
    /**
     * @param {[number, number, Uint8Array][]} pixMapArray
     * @returns {any}
     */
    _getPixbuf(pixMapArray) {
        if (!pixMapArray)
            return;
        ...
        return GdkPixbuf.Pixbuf.new_from_bytes(new GLib.Bytes(array), GdkPixbuf.Colorspace.RGB, true, 8, pixMap[0], pixMap[1], pixMap[0] * 4);
    }
}
...
export class SystemTray extends Service {
    ...
    /** @default (name: string) => this._items.get(name) */
    getItem = (name) => this._items.get(name);
     ...
     */
    _register() {
        Gio.bus_own_name(Gio.BusType.SESSION, 'org.kde.StatusNotifierWatcher', Gio.BusNameOwnerFlags.NONE, (connection) => {
    ...
    RegisterStatusNotifierItemAsync(serviceName, invocation) {
        ...
        item.connect('ready', () => {
            ...
            this.notify('items');
            this.emit('changed');
            this._dbus.emit_signal('StatusNotifierItemRegistered', new GLib.Variant('(s)', [busName + objectPath]));
        });
        item.connect('removed', () => {
            ...
            this.notify('items');
            this.emit('changed');
            this._dbus.emit_signal('StatusNotifierItemUnregistered', new GLib.Variant('(s)', [busName]));
        });
    }
}
