...
import { bulkConnect } from  'resource:///com/github/Aylur/ags/utils.js'; //'../utils.js';
...
export class Wifi extends Service {
    ...
    constructor(client: NM.Client, device: NM.DeviceWifi) {
        ...
        if (this._device) {
            bulkConnect((this._device as unknown) as Service, [
                ...
                ['access-point-added', () => this._notify()],
                ['access-point-removed', () => this._notify()],
            ]);
            this._activeAp();
        }
    }
    ...
    private _activeAp() {
        ...
        this._apBind = this._ap.connect('notify::strength', this._notify.bind(this));
    }
    ...
    get tooltip() {
        if (this.internet === 'connected') {
            return `${this.ssid} - ${this.strength}%`
        }
